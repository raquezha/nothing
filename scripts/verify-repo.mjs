...
`);
    ...
    writeFileSync(argsFile, "");
    result = run("bash", ["-c", `source ${JSON.stringify(path.join(root, "dotfiles/shell_integration.sh"))}; tmp=$(mktemp -d); cd "$tmp"; git init -b main >/dev/null 2>&1; pi --nochestra /triage local:shell-proof`], root, { env: nochestraDeliveryEnv });
