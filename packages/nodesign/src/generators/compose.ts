...
export function toComposeColor(
  hex?: string,
  colorTokens?: ColorTokenFact[]
): string {
  ...
  if (colorTokens && colorTokens.length > 0) {
    const matchedToken = colorTokens.find(
      (ct) => ct.hex.toUpperCase() === cleanHex
    );
...
export function formatComposePadding(
  padStr: string,
...
export function nodeToCompose(
  ...
  if (node.text) {
    ...
    const weightLine = fontWeight
      ...
      : "";
    const escapedText = escapeString(node.text);
    return `${pad}// ${name}\n${pad}Text(\n${pad}    text = "${escapedText}"${fontSizeLine}${weightLine},\n${pad}    color = ${color}\n${pad})`;
  }
  ...
  const isFullWidth =
    node.layout?.width &&
    (node.layout.width >= 350 ||
      node.layout.width === 360 ||
      node.layout.width === 390 ||
      node.layout.width === 412);
  ...
  const inner = children
    .map((c: UnifiedNode) =>
      nodeToCompose(c, indent + 1, context, rootScreenName)
    )
    .join("\n");
