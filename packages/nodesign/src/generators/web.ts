...
export function nodeToReact(node: UnifiedNode, indent = 1): string {
  ...
  const children: UnifiedNode[] = Array.isArray(node.children)
    ? node.children
    : [];
  const isRow =
    node.layout?.direction === "ROW" || node.layout?.direction === "HORIZONTAL";
  ...
  if (node.layout?.width && node.layout?.height)
    styles.push(`w-[${node.layout.width}px] h-[${node.layout.height}px]`);

  const styleAttr = node.color
    ? ` style={{ backgroundColor: '${node.color}' }}`
    : "";

  if (node.text) {
    const fontStyle = node.font?.fontSize
      ? ` text-[${node.font.fontSize}px]`
      : "";
    const colorStyle = node.color ? ` style={{ color: '${node.color}' }}` : "";
    return `${pad}<span className="${fontStyle.trim()}"${colorStyle}>${escapeHtml(
      node.text
    )}</span>`;
  }

  const inner = children
    .map((c: UnifiedNode) => nodeToReact(c, indent + 1))
    .join("\n");
...
export function nodeToHtml(node: UnifiedNode, indent = 1): string {
  ...
  if (node.text) {
    ...
    if (node.font?.fontFamily)
      fontStyles.push(`font-family: '${node.font.fontFamily}'`);
    if (node.font?.fontSize)
      fontStyles.push(`font-size: ${node.font.fontSize}px`);
    if (node.font?.fontWeight)
      fontStyles.push(`font-weight: ${node.font.fontWeight}`);
    if (node.color) fontStyles.push(`color: ${node.color}`);
    return `${pad}<span style="${fontStyles.join("; ")}">${escapeHtml(
      node.text
    )}</span>`;
  }

  const inner = children
    .map((c: UnifiedNode) => nodeToHtml(c, indent + 1))
    .join("\n");
