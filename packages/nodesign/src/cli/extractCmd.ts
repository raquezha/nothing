...
import {
  generateGroundingManifest,
  formatGroundingManifestMarkdown,
} from "../manifest.js";
...
export async function handleExtractCommand(
  ...
  const defaultDir = taskPath
    ? path.join(taskPath, "evidence")
    : path.resolve(process.cwd(), "design-renders");
  const outputDir = args.out
    ...
    : undefined;
  const zeplin =
    parsed.link.provider === "zeplin"
      ? await resolveZeplinScreen(
          parsed.link.url,
          undefined,
          outputDir,
          fetchFn
      ...
      : undefined;
  const figma =
    parsed.link.provider === "figma"
      ? await resolveFigmaLink(
          parsed.link.url,
          undefined,
          outputDir,
          fetchFn,
  ...
  const hierarchy =
    zeplin?.extract?.hierarchy || figma?.extract?.hierarchy || [];
  const extractedColors =
    zeplin?.extract?.colors ||
    figma?.extract?.colors ||
    zeplin?.screen?.colors ||
    [];
  ...
  const archDetailNote =
    inspection.notes.find((n) =>
      n.startsWith("Project Architecture Structure:")
    ) || inspection.architectureType;
  const manifest = generateGroundingManifest(
    hierarchy,
    screenName,
    codeContext,
    extractedColors
  ...
  if (args.manifest) {
    if (providerResult && providerResult.status !== "SUCCESS") {
      const errNote =
        providerResult.errorDescription ||
        providerResult.note ||
        `Provider status: ${providerResult.status}`;
      if (args.json) {
        console.log(
          JSON.stringify(
            { status: providerResult.status, error: errNote, manifest: null },
            null,
            2
          )
        );
        ...
        console.error(
          `\x1b[31mError (${providerResult.status}):\x1b[0m ${errNote}`
        );
      }
      return;
    }
  ...
  if (args.json) {
    console.log(
      JSON.stringify(
        {
          ...parsed,
          directive: formatAgentDirective(
            screenName,
            inspection.architectureType,
            archDetailNote,
            inspection.androidUIStack
          ),
          manifest,
          ...(zeplin ? { zeplin } : {}),
          ...(figma ? { figma } : {}),
  ...
  if (args.markdown) {
    ...
    if (args.code) {
      ...
      if (hierarchy.length === 0) {
        console.log(
          `// No child layers or UI elements found in frame '${screenName}'. Select a frame containing UI elements to generate code.`
        );
      } else {
        console.log(
          generateCodeSnippet(hierarchy, args.code, screenName, codeContext)
        );
      }
      console.log("```");
    }
  ...
  console.log(
    `${statusColor(providerResult?.status || parsed.status)}◆${color.reset} ${
      color.bold
    }Status${color.reset} ${color.dim}›${color.reset} ${
      providerResult?.status || parsed.status
    }`
  );
  ...
  if (zeplin) {
    ...
    if (zeplin.screen?.colors.length)
      printList(
        "Colors",
        zeplin.screen.colors.map((c: any) => c.hex)
      );
    if (zeplin.extract?.typography.length)
      printList(
        "Typography",
        zeplin.extract.typography.map(
          (t: any) => `${t.fontFamily || "font"} ${t.fontSize || ""}px`
        )
      );
    if (zeplin.extract?.layout.width || zeplin.extract?.layout.height)
      printStep(
        "Layout",
        `${zeplin.extract.layout.width || 0}x${
          zeplin.extract.layout.height || 0
        }`
      );
    if (zeplin.extract?.hierarchy.length)
      printList(
        "UI Blueprint",
        formatTreeBlueprint(zeplin.extract.hierarchy, 1)
      );
    ...
    printNote(
      "Zeplin",
      zeplin.note && zeplin.note !== zeplin.errorDescription
        ? zeplin.note
        : undefined
    );
  }
  if (figma) {
    printStep(
      "File",
      figma.fileKey
        ? `${figma.fileKey}${figma.nodeId ? ` (Node ID: ${figma.nodeId})` : ""}`
        : undefined
    );
    printStep("Name", figma.name);
    if (figma.extract?.colors.length)
      printList(
        "Colors",
        figma.extract.colors.map((c: any) => c.hex)
      );
    if (figma.extract?.typography.length)
      printList(
        "Typography",
        figma.extract.typography.map(
          (t: any) => `${t.fontFamily || "font"} ${t.fontSize || ""}px`
        )
      );
    if (figma.extract?.layout.width || figma.extract?.layout.height)
      printStep(
        "Layout",
        `${figma.extract.layout.width || 0}x${figma.extract.layout.height || 0}`
      );
    if (figma.extract?.hierarchy.length)
      printList(
        "UI Blueprint",
        formatTreeBlueprint(figma.extract.hierarchy, 1)
      );
    ...
    printNote(
      "Figma",
      figma.note && figma.note !== figma.errorDescription
        ? figma.note
        : undefined
    );
  }
  console.log(`${color.cyan}│${color.reset}`);
  console.log(
    `${color.cyan}└${color.reset} ${color.dim}zero-drift contract active; use --markdown or --json for full agent payload${color.reset}`
  );
  if (args.code) {
    console.log(`\n${color.bold}Generated Code (${args.code})${color.reset}`);
    if (hierarchy.length === 0) {
      console.log(
        `${color.yellow}◆${color.reset} ${color.dim}No child layers found in frame '${screenName}'. Select a frame with UI content to generate code.${color.reset}`
      );
    } else {
      console.log(
        generateCodeSnippet(hierarchy, args.code, screenName, codeContext)
