...
export function statusColor(status: string): string {
  ...
  if (status.includes("NOT_FOUND") || status.includes("REQUIRED"))
    return color.yellow;
...
export function printStep(label: string, value?: string): void {
  if (!value) return;
  console.log(
    `${color.cyan}◇${color.reset} ${color.bold}${label}${color.reset} ${color.dim}›${color.reset} ${value}`
  );
}
...
export function printList(title: string, items: string[] = []): void {
  ...
  console.log(
    `${color.cyan}◇${color.reset} ${color.bold}${title}${color.reset}`
  );
  for (const item of items)
    console.log(
      `${color.cyan}│${color.reset}  ${color.dim}•${color.reset} ${item}`
    );
}
...
export function printNote(provider: string, note?: string): void {
  ...
  console.log(`${color.cyan}│${color.reset}`);
  console.log(
    `${color.yellow}◆${color.reset} ${color.bold}${provider} note${color.reset}`
  );
