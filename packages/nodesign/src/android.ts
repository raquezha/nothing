...
function inspectFiles(rootPath: string): AndroidInspection {
  ...
  const hasCommonMain = files.some((file) =>
    file.includes(`${path.sep}commonMain${path.sep}`)
  );
  const hasComposeResources = files.some((file) =>
  ...
  const hasKmpComposeGradle = gradleTexts.some((text) =>
    text.includes("org.jetbrains.compose")
  );
  ...
  const hasCompose = gradleTexts.some(
    (text) =>
      text.includes("androidx.compose") ||
  ...
  const hasAndroidManifest = files.some((file) =>
    file.endsWith("AndroidManifest.xml")
  );
  const hasAndroidGradlePlugin = gradleTexts.some(
    (text) =>
  ...
  if (
    hasComposeResources ||
    (hasCommonMain && (hasKmpComposeUsage || hasKmpComposeGradle))
  )
    androidUIStack = "kmp";
  ...
  else if (hasAndroidManifest || hasAndroidGradlePlugin)
    androidUIStack = "ambiguous";
  ...
  const notes: string[] = [];
  notes.push(
    `Project Architecture Structure: ${architectureType} (${archInfo.details})`
  );
  if (androidUIStack === "n/a")
    notes.push(`No Android or KMP UI signals detected in ${rootPath}`);
  if (androidUIStack === "ambiguous")
    notes.push(
      `Android project found in ${rootPath}, but Compose/XML/KMP signals are ambiguous`
    );
  if (components.length === 0)
    notes.push(`No reusable ui/components files detected in ${rootPath}`);
  else
    notes.push(
      `Found ${components.length} reusable UI component(s) in ${rootPath}`
    );
