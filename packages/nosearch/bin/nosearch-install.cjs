...
const bundleName = "nosearch";
const skillNames = ["brave-search", "firecrawl"];

function usage() {
  console.log(`Usage: nosearch-install [--target pi|claude|codex|all] [--dest PATH] [--copy] [--force] [--dry-run]

Installs the nosearch skill bundle for agent runtimes.

Targets:
  pi      Link skills into ~/.pi/agent/skills (default)
  claude  Link skills into ~/.claude/skills
  codex   Link skills into ~/.codex/skills/nosearch and generate an AGENTS.md adapter
...
function writeCodexAdapter(root, opts) {
  const adapter = path.join(root, "AGENTS.md");
  const body = `# nosearch skills for Codex\n\nCodex does not currently auto-load Pi/Claude SKILL.md bundles from npm.\n\nUse these installed skills as reference workflows when search is needed:\n\n- brave-search/SKILL.md — Brave Search API workflow\n- firecrawl/SKILL.md — Firecrawl scrape/search/map workflow\n\nIf your Codex environment supports AGENTS.md discovery, copy or reference this file from your project.\n`;
...
function installTarget(target, opts) {
  ...
  if (target === "codex") writeCodexAdapter(root, opts);
